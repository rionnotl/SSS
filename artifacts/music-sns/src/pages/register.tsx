...
export default function Register() {
  ...
  const onSubmit = (data: z.infer<typeof registerSchema>) => {
    registerMutation.mutate({ data }, {
      ...
      onError: (err: any) => {
        toast({
          title: "登録失敗",
          description: err?.error || "エラーが発生しました",
          variant: "destructive",
        });
      }
    });
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] bg-background p-4 relative overflow-hidden">
      {/* Decorative blobs */}
      <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-secondary/20 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-primary/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="w-full max-w-md bg-card/80 backdrop-blur-xl border border-white/10 p-8 rounded-2xl shadow-2xl relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">あなたの音楽の世界を始めよう</p>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-white">ユーザー名 (半角英数字)</FormLabel>
                  <FormControl>
        ...
        <div className="mt-8 text-center text-sm text-muted-foreground">
          すでにアカウントをお持ちですか？{" "}
          <Link href="/login" className="text-primary hover:text-primary/80 font-bold hover:underline">
            ログイン
          </Link>
        </div>
      </div>
